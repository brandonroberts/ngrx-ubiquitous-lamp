 ...
 */
export interface RouterNavigationPayload<T> {
  routerState: T;
 ...
 * An action dispatched when the router navigates.
 */
export interface RouterNavigationAction<T = RouterStateSnapshot> {
 ...
 */
export interface RouterCancelPayload<T, V> {
  routerState: V;
  storeState: T;
 ...
 */
export interface RouterCancelAction<T, V = RouterStateSnapshot> {
 ...
 */
export interface RouterErrorPayload<T, V> {
  routerState: V;
  storeState: T;
 ...
 * An action dispatched when the router errors.
 */
export interface RouterErrorAction<T, V = RouterStateSnapshot> {
...
export interface RouterReducerState<T = RouterStateSnapshot> {
  state: T;
  navigationId: number;
}
...
export class StoreRouterConnectingModule {
  ...
  private setUpStoreStateListener(): void {
    ...
    this.store
      .pipe(select(this.stateKey), concatMap(() => this.navigateIfNeeded()))
      .subscribe();
  }
  ...
  private navigateIfNeeded() {
    ...
    if (this.router.url !== this.storeState[this.stateKey].state.url) {
      ...
      ).pipe(tap(() => (this.navigationTriggeredByDispatch = false)));
    }
  }
